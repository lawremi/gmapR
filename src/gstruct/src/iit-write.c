static char rcsid[] = "$Id: iit-write.c 68062 2012-07-03 23:33:23Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
static bool
is_sorted (int array[], int i, int j, unsigned int (*endpoint)(struct Interval_T *, int), struct Interval_T *intervals) {
  int lambda;
...
IIT_create_div_footer (T new, List_T divlist, List_T typelist, List_T fieldlist,
  ...
  if (new->total_nintervals == 0) {
    new->intervals[0] = (struct Interval_T *) NULL;
    for (divno = 1; divno < new->ndivs; divno++) {
      new->intervals[divno] = (struct Interval_T *) NULL;
    }
  } else {
