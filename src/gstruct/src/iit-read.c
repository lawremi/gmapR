static char rcsid[] = "$Id: iit-read.c 58844 2012-03-02 01:05:02Z twu $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
IIT_data (T this, int index) {
  return this->datapointers[index-1];
}
...
IIT_free (T *old) {
  ...
  if (*old != NULL) {
    ...
    } else if ((*old)->access == CREATED) {
      FREE((*old)->labels);
      FREE((*old)->labelorder);
      FREE((*old)->labelpointers);
      if ((*old)->datapointers != NULL) {
	for (k = 0; k < (*old)->total_nintervals; k++) {
	  FREE((*old)->datapointers[k]);
	}
	FREE((*old)->datapointers);
      }
...
IIT_contained (T this, char *divstring, unsigned int x, unsigned int y) {
  ...
  if (divno < 0) {
    ...
    for (lambda = min1; lambda <= max2; lambda++) {
      match = this->sigmas[divno][lambda];
      if (Interval_contained_p(x,y,this->intervals[divno],match) == true) {
	return true;
      }
      match = this->omegas[divno][lambda];
      if (Interval_contained_p(x,y,this->intervals[divno],match) == true) {
...
IIT_exists_with_divno_typed_signed (T this, int divno, unsigned int x, unsigned int y, int type, int sign) {
  ...
  if (divno < 0) {
    ...
    for (lambda = min1; lambda <= max2; lambda++) {
      match = this->sigmas[divno][lambda];
      interval = &(this->intervals[divno][match]);
      if (Interval_overlap_p(x,y,this->intervals[divno],match) == true &&
  ...
  *nexactmatches = 0;
  ...
  if (*nexactmatches == 0) {
    ...
    for (i = 0; i < nmatches; i++) {
      ...
      interval = &(this->intervals[0][index-1]);
      if (Interval_low(interval) == x && Interval_high(interval) == y) {
	exactmatches[j++] = Interval_type(interval);
      }
    }
    FREE(matches);
    return exactmatches;
